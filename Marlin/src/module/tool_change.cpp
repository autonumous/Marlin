...
#if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
  inline void switching_toolhead_tool_change(const uint8_t tmp_extruder, const float fr_mm_s/*=0.0*/, bool move/*=false*/) {
    if (!move) return;
#else  
...
#if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
  inline void magnetic_switching_toolhead_tool_change(const uint8_t tmp_extruder, const float fr_mm_s/*=0.0*/, bool move/*=false*/) {
    if (!move) return;
#else  
...
inline void invalid_extruder_error(const uint8_t e) {
  ...
  inline void dualx_tool_change(const uint8_t tmp_extruder, bool &no_move) {
    ...
    if (dual_x_carriage_mode == DXC_AUTO_PARK_MODE
        && IsRunning()
 #if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
        && (delayed_move_time || current_position[X_AXIS] != xhome) && move
#else  
...
void tool_change(const uint8_t tmp_extruder, const float fr_mm_s/*=0.0*/, bool no_move/*=false*/) {
    ...
    if (tmp_extruder >= EXTRUDERS)
     ...
     #if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
      const bool should_swap = move && toolchange_settings.swap_length
     #else  
    ...
    if (tmp_extruder != active_extruder) {
      ...
      if (!no_move) {
       ...
       #if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
        dualx_tool_change(tmp_extruder, move);
       #else  
       ...
       #if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
        switching_toolhead_tool_change(tmp_extruder, fr_mm_s, move);
       #else  
       ...
       #if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
        magnetic_switching_toolhead_tool_change(tmp_extruder, fr_mm_s, move);
       #else  
       ...
       #if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
        if (move)planner.buffer_line(current_position, planner.settings.max_feedrate_mm_s[Z_AXIS], active_extruder);
       #else  
        if (!no_move)planner.buffer_line(current_position, planner.settings.max_feedrate_mm_s[Z_AXIS], active_extruder);
       #endif  //BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE

       #if ENABLED(BB_CUSTOM_TOOL_CHANGE_BEHAVIOUR_NOMOVE)
        if (!move) fast_line_to_current(Z_AXIS);
       #else  
